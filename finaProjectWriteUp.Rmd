...
For Cross Validation, we will be using 7-folds Cross Validation.

### Random Forest
First of all, we will build a Random Forest model and perform a prediction
```{r, cache=TRUE, message=FALSE}
mod.rf <- randomForest(classe ~., data = df.train, trControl = trainControl("cv", number = 7))
mod.rf
plot(mod.rf)
```
...
mod.rpart <- rpart(classe ~., data = df.train, method = "class",
                   control = rpart.control(method = "cv", number = 7))
mod.rpart
